...
static struct proc_dir_entry *oplusVersion = NULL;
 ...
 static int __init update_manifest(void)
{
	...
	if (oppoVersion && oplusVersion) {
		if (substr[0] == '0') {
			proc_symlink(manifest,oppoVersion,manifest_src[1]);		//single sim
			proc_symlink(manifest,oplusVersion,manifest_src[1]);	//single sim
		} else {
			proc_symlink(manifest,oppoVersion,manifest_src[0]);		//double sim
			proc_symlink(manifest,oplusVersion,manifest_src[0]);	//double sim
		}
	}
	set_fs(fs);
...
static int __init update_feature(void)
{
    ...
    if (oppoVersion && oplusVersion) {
			...
			proc_symlink(nfc_feature, oplusVersion, feature_src);
		}
	}
	set_fs(fs);
	return 0;
...
static int __init oppo_project_init(void)
{
        ...
        oplusVersion =  proc_mkdir("oplusVersion", NULL);
        if (!oppoVersion || !oplusVersion) {
...
/* Cong.Dai@psw.bsp.tp, 2019.06.15, add for init engineering version file*/
        ...
        //oplusVersion
        pentry = proc_create("prjName", S_IRUGO, oplusVersion, &prjVersion_proc_fops);
        if (!pentry) {
                pr_err("create prjVersion proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
        pentry = proc_create("pcbVersion", S_IRUGO, oplusVersion, &pcbVersion_proc_fops);
        if (!pentry) {
                pr_err("create pcbVersion proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
        pentry = proc_create("operatorName", S_IRUGO, oplusVersion, &operatorName_proc_fops);
        if (!pentry) {
                pr_err("create operatorName proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
        pentry = proc_create("modemType", S_IRUGO, oplusVersion, &modemType_proc_fops);
        if (!pentry) {
                pr_err("create modemType proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
        pentry = proc_create("oplusBootmode", S_IRUGO, oplusVersion, &oppoBootmode_proc_fops);
        if (!pentry) {
                pr_err("create oplusBootmode proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
        pentry = proc_create("secureType", S_IRUGO, oplusVersion, &secureType_proc_fops);
        if (!pentry) {
                pr_err("create secureType proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
...
#ifdef VENDOR_EDIT
/*Ziqing.Guo@BSP.Fingerprint.Secure 2017/04/16 Add for distinguishing secureboot stage*/
        pentry = proc_create("secureStage", S_IRUGO, oplusVersion, &secureStage_proc_fops);
        if (!pentry) {
                pr_err("create secureStage proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
#endif /* VENDOR_EDIT */
        pentry = proc_create("serialID", S_IRUGO, oplusVersion, &serialID_proc_fops);
        if (!pentry) {
                pr_err("create serialID proc failed.\n");
                goto ERROR_INIT_VERSION;
        }

        pentry = proc_create("ocp", S_IRUGO, oplusVersion, &ocp_proc_fops);
        if (!pentry) {
                pr_err("create serialID proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
...
#ifdef VENDOR_EDIT
/* Cong.Dai@psw.bsp.tp, 2019.06.15, add for init engineering version file*/
        pentry = proc_create("engVersion", S_IRUGO, oplusVersion, &eng_version_proc_fops);
        if (!pentry) {
                pr_err("create engVersion proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
        pentry = proc_create("isConfidential", S_IRUGO, oplusVersion, &confidential_proc_fops);
        if (!pentry) {
                pr_err("create isConfidential proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
